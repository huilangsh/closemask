...
```json
{
  ...
  "port": 8846,
  "oneaifw_url": "http://localhost:8844",
  "storage_type": "redis",
...
> **Note**: LLM API keys are NOT configured here. They are passed via the `Authorization` header in requests and automatically forwarded to the LLM by CloseMask.
...
docker run -d \
  --name closemask \
  -p 8846:8846 \
  -e CLOSEMASK_LLM_URL=https://api.openai.com/v1 \
  closemask:latest
```
  ...
  config.json: |
    {
      "llm_url": "https://api.openai.com/v1",
      "port": 8846,
      "oneaifw_url": "http://oneaifw-service:8844",
      "storage_type": "redis",
...
| `CLOSEMASK_API_KEY` | CloseMask auth key (`X-CloseMask-Key` header) | - |
