...
When you use third-party LLM APIs (OpenAI, Anthropic, Azure, etc.), all request data leaves your infrastructure and is sent to external servers. This creates critical privacy and security risks:

- **Credential Leaks**: Developers may accidentally send API keys, database passwords, and other technical credentials to LLMs during debugging
- **User Privacy Exposure**: Customer service conversations contain phone numbers, ID cards, bank account numbers, and other PII sent to third parties
- **Corporate Data Breach**: Employees inadvertently share code or configuration containing sensitive information with AI assistants
- **Compliance Violations**: Cross-border PII transmission may violate GDPR, PIPL, CCPA, and other data protection regulations

CloseMask builds a protective barrier between you and third-party APIs — all sensitive data is replaced with placeholders before leaving your infrastructure, LLMs only see sanitized data, and placeholders in responses are automatically restored to original values. The entire process is transparent to users and agents.
...
- Multi-provider support: OpenAI, Anthropic (via OpenAI-compatible proxy), Azure OpenAI, Ollama, DeepSeek
...
User: "My ID is 110101199003077777"
    ↓
CloseMask detects ID → masks to ${ID_CARD_xxx}
    ↓
Sends to LLM: "My ID is ${ID_CARD_xxx}"
    ↓
...
CloseMask is built on top of OneAIFW PII detection engine, providing an agent-ready layer with critical workflow features:
